...
export class OceanCurrentsLayer implements ILayer {
  readonly metadata: LayerMetadata = {
    id: 'layer-ocean-currents',
    name: 'Ocean Currents',
    category: LayerCategory.Data,
    icon: '🌀',
    description: 'Ocean current vectors — requires backend proxy',
  ...
  enable(): void {
    this.enabled = true;
    if (this.engine) {
      void this.engine.query({ variable: EnvironmentalVariable.OceanCurrentSpeed });
    }
  }
